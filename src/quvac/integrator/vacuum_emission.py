...
class VacuumEmission(object):
    ...
    def calculate_one_time_step(self, t, weight=1):
        ...
        if not self.channels:
            ...
            self.field.calculate_probe_pump_fields(t, E_probe=self.E_probe, B_probe=self.B_probe,
...
class VacuumEmissionProbePump(object):
    '''
    Calculator of Vacuum Emission amplitude for probe channel 
    from given fields
    ...
    def allocate_fields(self):
        self.E_probe = [np.zeros(self.grid_shape, dtype=np.complex128) for _ in range(3)]
        self.B_probe = [np.zeros(self.grid_shape, dtype=np.complex128) for _ in range(3)]
        self.E_pump = [np.zeros(self.grid_shape, dtype=np.complex128) for _ in range(3)]
        self.B_pump = [np.zeros(self.grid_shape, dtype=np.complex128) for _ in range(3)]
    ...
    def free_resources(self):
        del self.E_probe, self.B_probe, self.E_pump, self.B_pump
        del self.tmp, self.tmp_fftw
    ...
    def calculate_one_time_step(self, t, weight=1):
        ...
        self.allocate_fields()
        self.field.calculate_probe_pump_fields(t, E_probe=self.E_probe, B_probe=self.B_probe,
                                               E_pump=self.E_pump, B_pump=self.B_pump)
        Ex, Ey, Ez = [E.real for E in self.E_pump]
        Bx, By, Bz = [B.real for B in self.B_pump]
        Epx, Epy, Epz = [E.real for E in self.E_probe]
        Bpx, Bpy, Bpz = [B.real for B in self.B_probe]
